...
The summary states that a license lasts one year from creation. Each row offers password-confirmed key viewing with a copy action. Older keys that cannot be displayed show a short explanation and an explicit replacement action; there is no separate key-save task. New and replacement keys are stored automatically without changing the license expiry. The server / IP change form explains that the same key and expiry date are preserved. Key replacement remains a separate action with its own explanation and confirmation. Keep these choices distinct in labels and form headings. Revealed keys appear in a bordered white block with wrapping mono text and a copy-result status announcement.
