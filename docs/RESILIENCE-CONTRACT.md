...
| P0.5 | Open — documented renewal/firewall dependencies remain | [Independent firewall source and Debian AR reboot](../deploy/e2e/release-recovery/FIREWALL-BOOT.md) pass with management binaries absent. The installed unit still invokes Agent. Shared Agent/consumer exclusion now has source and native contention evidence; packaging and migration remain open. Required: removal/absence and reboot probes for each claimed native workload combination. |
