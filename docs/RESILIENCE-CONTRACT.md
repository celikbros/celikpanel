...
| P0.2 | Partial — typed access and Agent-independent startup observation implemented | [Access/observation acceptance](RECOVERY-ACCESS.md): real panel process without Agent, bounded token reads, strict native observations, and browser fault cases. Panel binary, DB, TLS and eager shell remain dependencies; full native interruption/reboot acceptance stays open. |
| P0.3 | Partial — snapshot-bound unit transition recovery implemented and natively tested | [Scoped implementation and acceptance](../deploy/e2e/release-recovery/UNIT-TRANSITION.md): atomic old/candidate publication and restoration, owner-edit refusal, and phase-appropriate guard checks. The separate versioned executor, complete checkpoints, interrupted rollback and reboot coverage remain open. |
