...
| P0.5 | Partial — independent packaged firewall and Debian native update/rollback; renewal and wider matrix open | [Management-absent boot](../deploy/e2e/release-recovery/FIREWALL-BOOT.md), [retained A/B/A generations](../deploy/e2e/release-recovery/FIREWALL-GENERATIONS.md), and [actual Debian signed-worker update/automatic rollback](../deploy/e2e/release-recovery/FIREWALL-UPDATE.md) have bounded evidence. Packaged units use the independent retained helper; historical/direct-source units are not silently rewritten. Open: production UI/trust, Arch, failed-helper boot/console recovery, independent renewal, and each claimed native workload/absence combination. |
