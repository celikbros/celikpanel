...
for index,name in enumerate(['dnsprimary','dnssecondary'],1):
    ...
    port=2283+index; mac=f'52:54:00:69:00:{index:02d}'; peer_mac=f'52:54:00:69:01:{index:02d}'; peer_ip='192.0.2.10' if index==1 else '192.0.2.20'
    ...
    command.extend(['-netdev', 'socket,id=peer,'+('listen' if index==1 else 'connect')+'=127.0.0.1:2280','-device','virtio-net-pci,netdev=peer,mac='+peer_mac])
    run(command)
