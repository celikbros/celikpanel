...
assert ROOT.name=='cp-setup-dns-profile-20260911'
...
if primary:
    ...
    assert created.get('domain_id',0)>0 and not created.get('dns_pending',False),created
    answers={}
    for host in ('192.0.2.10','192.0.2.20'):
        for kind in ('SOA','A'):
            ...
            assert result.returncode==0 and 'status: NOERROR' in result.stdout and ' aa;' in result.stdout,result.stdout+result.stderr
            answers[host+' '+kind]=result.stdout
            if kind=='A':assert '192.0.2.10' in result.stdout,result.stdout
    soa=[]
    for host in ('192.0.2.10','192.0.2.20'):
        lines=[line for line in answers[host+' SOA'].splitlines() if line and not line.startswith(';')]
        assert len(lines)==1,lines
        soa.append(lines[0].split()[4:])
    assert soa[0]==soa[1],soa
