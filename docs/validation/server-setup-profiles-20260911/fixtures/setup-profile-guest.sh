...
apt-get update
apt-get install -y dnsmasq-base ca-certificates openssl curl
...
address=/panel.__PROFILE__.setup.test/10.0.2.15
...
[Service]
ExecStart=/var/lib/celikpanel-profile-vm/pebble -config /var/lib/celikpanel-profile-vm/pebble.json -dnsserver 127.0.0.1:53 -strict=false
