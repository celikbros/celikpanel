...
| P0.5 | Açık — belgelenmiş yenileme/güvenlik duvarı bağımlılıkları sürüyor | [Bağımsız firewall kaynağı ve Debian AR yeniden başlatması](../deploy/e2e/release-recovery/FIREWALL-BOOT.md), yönetim binary’leri yokken geçti. Kurulu unit hâlâ Agent kullanır. Ortak Agent/okuyucu kilidinin kaynak ve yerel çakışma kanıtı var; paketleme ve geçiş açıktır. Gerekli: desteklendiği söylenen her yerel hizmet birleşimi için yönetim bileşenlerinin kaldırılması/yokluğu ve yeniden başlatma kontrolleri. |
