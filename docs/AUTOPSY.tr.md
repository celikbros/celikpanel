...
| B1 | **Tek API**: v2'yi v1'e katla; kiracı kapsamı auth'tan; OpenAPI üret; ön yüz tipleri üretilen istemciden | A3+A5 sınıfı hatalar derlemede ölür; 74 ham `fetch(` tek katmana iner; "API-first" sözü gerçek olur | 3-5 gün | 🔶 Kısmi (17 Tem): A3 kiracı kapsamı + A4 parola şifreleme + sunucu-kaydı rol ayrımı/admin kilidi KAPANDI (canlı rol-matrisi testiyle). Kalan alt dilimler: v2→v1 yol birleştirme · OpenAPI + üretilen istemci · hata sözleşmesi {code,hint,action} |
