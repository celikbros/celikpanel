...
| A2 | Müşteri/bayi Databases sayfası görüyor (nav ALL) ama `/api/v2/` tümüyle admin kilidinde → admin olmayana her zaman kırık sayfa | `nav.ts:32` ↔ `middleware.go:141` | ✅ Geçici kapandı (11 Tem): nav yalnız admin. **Kalıcı çözüm B1'e bağlı** |
...
| A4 | DB sunucu root parolası düz metin: `// TODO: Encrypt` | `database_v2_handlers.go:138` | ⬜ AÇIK — anahtar-yönetimi kararı gerektirir (nerede saklanır?); ayrı odaklı commit, B1'in kalan dilimi |
...
| B1 | **Tek API**: v2'yi v1'e katla; kiracı kapsamı auth'tan; OpenAPI üret; ön yüz tipleri üretilen istemciden | A3+A5 sınıfı hatalar derlemede ölür; 74 ham `fetch(` tek katmana iner; "API-first" sözü gerçek olur | 3-5 gün | 🔶 Kısmi (11 Tem): A3 kiracı kapsamı KAPANDI. Kalan alt dilimler: A4 parola şifreleme · sunucu-kaydı rol ayrımı + admin kilidini kaldır · v2→v1 yol birleştirme · OpenAPI + üretilen istemci |
