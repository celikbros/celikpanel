...
> **Gerçeklik kontrolü (5 Temmuz 2026, gece):** `install.sh` bu geliştirme makinesinde **root ile gerçekten koşuldu** ve golden path **uçtan uca doğrulandı**: kurulum → systemd altında root agent + düşük yetkili panel → plan → müşteri → domain (sistem kullanıcısı, izin düzeni, nginx vhost, PHP-FPM havuzu) → canlı PHP sayfası → özel sertifikayla çalışan HTTPS → MariaDB'de gerçek veritabanı (bağlan+yaz) → IMAP/FTP girişleri → DNS zone şablonu → silmede tam sistem temizliği → idempotent yeniden kurulum (yükseltme). Bulunan ve düzeltilen gerçek hatalar: apt-get update kurulumun ölüm nedeniydi, www-data site dizinlerine giremiyordu (izin düzeni yazıldı), SSL sertifikası diske düşüyor ama nginx'e hiç bağlanmıyordu, DeleteSite boş bir taslaktı. **Hâlâ VPS isteyen kanıtlar:** temiz-Ubuntu ilk kurulumu (bu makinede servisler zaten kuruluydu), Let's Encrypt (kamusal DNS + 80 portu ister), PowerDNS'in kayıtlarımızı 53'te gerçekten sunması, gerçek mail teslimatı (Postfix/Dovecot uçtan uca) ve panelden servis kurmanın gerçek apt kurulumu.
...
- ❌ BSD desteği (şimdilik) — **ama seçenek bilinçli olarak korunuyor ve asla fork olarak değil.** Panel↔agent RPC sözleşmesi tasarım gereği OS-nötr: panel (HTTP/SQLite/UI/iş mantığı) taşınabilir Go'dur ve bugün FreeBSD'ye çapraz derlenir; yalnız agent'ın "elleri" (systemd/apt/nftables) Linux'a özgüdür. Gerçek talep doğarsa (örn. Linux'ta bir güven krizi barındırıcıları BSD'ye iterse) yapılacak iş, aynı RPC yüzeyinin arkasına bir BSD agent arka ucu yazmaktır — haftalık iş, tek ürün. İki CelikPanel asla olmayacak. Bunu ucuz tutan disiplin: yeni agent özellikleri "ne"yi (RPC yüzeyi) "nasıl"dan (exec çağrıları) ayrı tutar — ki kod zaten böyle yazılıyor. *(Karar: 8 Temmuz 2026.)*
