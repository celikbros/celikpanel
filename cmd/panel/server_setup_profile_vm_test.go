...
func runServerSetupDisposableProfileDaemon(t *testing.T, profile string) {
	...
	mux.HandleFunc(serverSetupPath+"/publisher", p.handleServerSetupPublisher)
	mux.HandleFunc("/api/v1/dns/remote/enrollments", p.handleRemoteDNSAdmin)
	mux.HandleFunc("/api/v1/dns/remote/clients", p.handleRemoteDNSAdmin)
	mux.HandleFunc("/api/v1/dns/remote/connections", p.handleRemoteDNSAdmin)
	mux.HandleFunc("/api/v1/dns/remote/accept", p.handleRemoteDNSMachine)
	mux.HandleFunc("/api/v1/dns/remote/receiver/status", p.handleRemoteDNSMachine)
	mux.HandleFunc("/api/v1/dns/remote/receiver/publish", p.handleRemoteDNSMachine)
