...
func TestRecoveryWaitingHTTPRemainsAnExactAdminRead(t *testing.T) {
	p, token := newAuthHandlerTestPanel(t, true)
	calls := 0
	read := func(id string) recoveryobs.Status {
		calls++
		...
		return recoveryobs.Status{Schema: recoveryobs.StatusSchema, RequestID: id, Observation: "known", Phase: "recovering", TerminalProof: "none", Reason: "recovery_running", WaitingFor: "starting", ObservedAt: "2026-09-16T13:36:15Z", PreviousFailure: "update_failed"}
	}
	handler := p.requireAuth(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { p.serveRecoveryStatus(w, r, read) }))
	...
	if rr.Code != 200 || calls != 1 || body["waiting_for"] != "starting" || body["terminal_proof"] != "none" || body["previous_failure"] != "update_failed" || len(body) != 10 || rr.Header().Get("Cache-Control") != "no-store" {
