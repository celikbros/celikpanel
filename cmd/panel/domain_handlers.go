...
func (p *Panel) handleDeleteDomain(w http.ResponseWriter, r *http.Request) {
	...
	if hasSite && projectType != "dnsonly" {
		if err := hostingpath.ValidateDocumentRoot(
			docroot, domain.SubscriptionID, domainID,
		); err != nil {
			writeClientError(w, http.StatusConflict,
				"this legacy site has a non-standard document root and cannot be deleted automatically; move it into the managed site directory first")
			return
		}
	}
