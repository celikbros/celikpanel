...
func (p *Panel) handleDomainMail(w http.ResponseWriter, r *http.Request) {
	...
	case strings.HasSuffix(r.URL.Path, "/mail/setup"):
		...
		p.handleMailClientSetup(w, domain)
...
func loadAllForwardings(ctx context.Context, queryer mailForwardingQueryer) ([]transport.MailForwarding, error) {
	...
	rows, err := queryer.QueryContext(ctx, "SELECT source, destination FROM email_forwardings ORDER BY id")
	if err != nil {
		return nil, err
	}
	for rows.Next() {
	...
	catchAllRows, err := queryer.QueryContext(ctx,
		"SELECT '@' || d.name, c.destination FROM mail_catch_all c JOIN domains d ON d.id = c.domain_id ORDER BY c.domain_id")
	if err != nil {
		return nil, err
	}
...
func (p *Panel) mutateForwardings(ctx context.Context, mutation func(*sql.Tx) error) error {
