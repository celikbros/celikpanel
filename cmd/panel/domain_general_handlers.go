...
func (p *Panel) handleGetGeneralSettings(w http.ResponseWriter, r *http.Request, domainID int) {
	...
	`, domainID).Scan(&site.DocumentRoot, &site.WebServer)
...
func (p *Panel) handleUpdateGeneralSettings(w http.ResponseWriter, r *http.Request, domainID int) {
	// The site root and web server are identity-derived, privileged runtime
	// properties. They are deliberately read-only here: accepting arbitrary
	// paths would let a tenant point nginx and the root agent at another
	// subscription. HTTPS is managed by the SSL/TLS endpoint, aliases by the
	// aliases endpoint.
	writeClientError(
		w,
		http.StatusMethodNotAllowed,
		"document root and web server are managed by the hosting layout; use SSL/TLS for HTTPS and the aliases endpoint for aliases",
	)
}
