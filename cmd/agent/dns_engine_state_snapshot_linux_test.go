...
func TestDNSEngineStateSnapshotRestoreRecreatesExactServiceOwner(t *testing.T) {
	if os.Geteuid() != 0 || os.Getegid() != 0 {
		t.Skip("exact root:service-group state ownership test requires root:root")
	}
	previousUID := serviceMutationRequiredOwnerUID
	...
	if err := restoreDNSEngineStateSnapshot(snapshot); err != nil {
		t.Fatalf("recreate root:service-group DNS state: %v", err)
	}
	actual, err := captureDNSEngineStateSnapshot(false)
	if err != nil {
		t.Fatal(err)
	...
	if !reflect.DeepEqual(actual, snapshot) {
		t.Fatalf("recreated DNS state differs:\n got: %+v\nwant: %+v", actual, snapshot)
	}
}

func TestWriteDNSEngineStateCreatesExactServiceOwner(t *testing.T) {
	if os.Geteuid() != 0 || os.Getegid() != 0 {
		t.Skip("exact root:service-group state ownership test requires root:root")
	}
	previousUID := serviceMutationRequiredOwnerUID
	...
	if err := writeDNSEngineState(state); err != nil {
		t.Fatalf("create root:service-group DNS state: %v", err)
	}
	snapshot, err := captureDNSEngineStateSnapshot(false)
	if err != nil {
		t.Fatal(err)
	...
	if !snapshot.OwnerKnown || snapshot.UID != 0 || snapshot.GID != 1 ||
		snapshot.Mode != 0o600 {
		t.Fatalf("created DNS state metadata differs: %+v", snapshot)
	}
	actual, exists, err := readDNSEngineState()
	if err != nil {
		t.Fatal(err)
	}
	if !exists || !reflect.DeepEqual(actual, state) {
		t.Fatalf("created DNS state = %+v exists=%t, want %+v", actual, exists, state)
	}
}

func TestPersistExactDNSEngineStateRejectsWrongOwnerReadback(t *testing.T) {
	if os.Geteuid() != 0 || os.Getegid() != 0 {
		t.Skip("exact root:service-group state ownership test requires root:root")
	}
	previousUID := serviceMutationRequiredOwnerUID
	...
	mustChownMode(t, path, 0, 0, 0o600)

	if err := persistExactDNSEngineState(state); err == nil {
		t.Fatal("byte-identical wrong-owner DNS state was accepted as exact readback")
	}
	...
	if !ok || stat.Uid != 0 || stat.Gid != 0 || info.Mode().Perm() != 0o600 {
		t.Fatalf("rejected wrong-owner DNS state metadata changed: %+v", info.Sys())
	}
	...
	if !bytes.Equal(data, encoded) {
		t.Fatal("rejected wrong-owner DNS state bytes changed")
	}
}
