...
require_literal 'needs: [go, panel-race, web, linux-arm64-compile, publisher-windows, membership]'
...
require_literal 'Build mandatory operator-authorized signed linux/amd64 assets'
require_literal '[[ "$signed_count" -eq 4 ]]'
require_literal 'assets=("$archive" "$checksum")'
require_literal 'assets+=("$signed_archive" "$signed_checksum" "$signed_manifest" "$signed_signature")'
require_literal '[[ "${#assets[@]}" -eq 6 ]]'
require_literal 'release asset set must contain exactly six files'
require_literal 'secrets.CELIKPANEL_RELEASE_SIGNING_ED25519_PEM'
require_literal 'vars.CELIKPANEL_RELEASE_SEQUENCE'
require_literal 'CELIKPANEL_RELEASE_SIGNING_ED25519_PEM is required for tagged releases'
require_literal 'grep -Fxq "bootstrap_release_version=${RELEASE_VERSION}" download-portal/get.sh'
require_literal 'grep -Fxq "bootstrap_release_sequence=${RELEASE_SEQUENCE}" download-portal/get.sh'
reject_literal 'signed assets skipped'
