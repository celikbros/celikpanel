#!/usr/bin/env bash
# Finalize only the guarded start/cleanup phase of one already-published normal
# update. The recovery release supplies reviewed guard logic; the original
# target release supplies the exact bytes already installed on the machine.
set -euo pipefail
...
usage() {
    printf '%s\n' \
        "usage: $0 --pending-target=<40-lowercase-hex> --recovery-commit=<40-lowercase-hex> --trusted-recovery-release=<absolute-immutable-release-root> --trusted-target-release=<absolute-immutable-release-root>" >&2
    exit 2
}
...
for argument in "$@"; do
    case "$argument" in
        ...
        --trusted-target-release=*)
            [[ -z "$TRUSTED_TARGET_RELEASE" ]] || usage
            TRUSTED_TARGET_RELEASE=${argument#--trusted-target-release=}
            ;;
        *) usage ;;
    esac
done
...
[[ "$TRUSTED_RECOVERY_RELEASE" == /* &&
   "$TRUSTED_TARGET_RELEASE" == /* ]] || usage
[[ "$RECOVERY_COMMIT" != "$PENDING_TARGET" &&
   "$TRUSTED_RECOVERY_RELEASE" != "$TRUSTED_TARGET_RELEASE" ]] \
    || die "recovery and pending-target provenance must be distinct"
[[ $EUID -eq 0 ]] || die "pending-update finalization must run as root"
...
validate_immutable_release() {
    ...
    local canonical relative entry owner group mode permissions version commit tree helper
    ...
    while IFS= read -r -d '' entry; do
        ...
        [[ "$owner" == 0 ]] \
            || die "$role release entry must be owned by root: $entry"
        permissions=$((8#$mode))
        (( (permissions & 0022) == 0 )) \
    ...
    if [[ "$role" == recovery ]]; then
        ...
        [[ "$helper" == "$canonical/$required_helper" ]] \
            || die "pending-update finalizer must execute from the verified recovery release"
    else
        [[ -f "$canonical/web/dist/index.html" &&
           ! -L "$canonical/web/dist/index.html" ]] \
            || die "target web artifact is missing or unsafe"
        TARGET_RELEASE_TREE=$tree
    fi
}
...
validate_pending_update_snapshot() {
    ...
    [[ "$target" == "$PENDING_TARGET" ]] \
        || die "pending update snapshot target differs from --pending-target"
    ...
    printf 'normal\n' | cmp -s - "$transition" \
        || die "completion-only finalizer accepts only a normal update snapshot"
...
verify_installed_release_artifacts() {
    ...
    cmp -s "$TRUSTED_TARGET_RELEASE/bin/panel" "$BIN_DIR/panel" \
        || die "installed panel does not match the pending target release"
    cmp -s "$TRUSTED_TARGET_RELEASE/bin/agent" "$BIN_DIR/agent" \
        || die "installed agent does not match the pending target release"
    ...
    cmp -s \
        <(cd "$TRUSTED_TARGET_RELEASE/web/dist" &&
            LC_ALL=C find . -mindepth 1 -printf '%y\t%p\n' | LC_ALL=C sort) \
        <(cd "$WEB_DIR" &&
            LC_ALL=C find . -mindepth 1 -printf '%y\t%p\n' | LC_ALL=C sort) \
        || die "installed web tree structure does not match the pending target"
    cmp -s \
        <(cd "$TRUSTED_TARGET_RELEASE/web/dist" &&
            LC_ALL=C find . -type f -print0 | LC_ALL=C sort -z | xargs -0 sha256sum) \
        <(cd "$WEB_DIR" &&
            LC_ALL=C find . -type f -print0 | LC_ALL=C sort -z | xargs -0 sha256sum) \
        || die "installed web tree does not match the pending target"
}
...
run_target_agent_idle_unlocked() {
    env -i PATH="$PATH" HOME=/root LC_ALL=C \
        ...
        "$TRUSTED_TARGET_RELEASE/bin/agent" --check-service-mutation-idle \
        || die "pending-target agent/package mutations are not idle"
}
...
run_target_agent_idle_locked() {
    ...
    env -i PATH="$PATH" HOME=/root LC_ALL=C \
        ...
        "$TRUSTED_TARGET_RELEASE/bin/agent" \
        --check-service-mutation-idle-under-external-lock \
        || die "pending-target agent mutation ledger is not idle under the external lock"
    verify_mutation_lock_held
}
...
run_target_panel_wal_idle() {
    env -i PATH="$PATH" HOME=/root LC_ALL=C \
        ...
        "$TRUSTED_TARGET_RELEASE/bin/panel" \
        --check-service-operations-idle-wal-aware \
        || die "pending-target panel operation ledger is not WAL-aware idle"
}
...
run_panel_migrations_offline() {
    ...
    sudo -u celikpanel -- env -i \
        PATH="$PATH" HOME="$PANEL_DATA_DIR" LC_ALL=C \
        CELIKPANEL_DATA_DIR="$PANEL_DATA_DIR" \
        "$BIN_DIR/panel" --migrate-only \
        || die "offline pending-target database migration failed"
    run_target_panel_wal_idle
...
wait_for_agent_ready() {
    ...
    systemctl start celikpanel-agent.service \
        || die "pending-target agent could not be started"
    ...
    die "pending-target agent did not become active with its exact socket"
}

wait_for_panel_ready() {
    local _
    systemctl start celikpanel-panel.service \
        || die "pending-target panel could not be started"
    for _ in $(seq 1 50); do
        systemctl is-active --quiet celikpanel-panel.service && return 0
        sleep 0.2
    done
    die "pending-target panel did not become active"
}
...
validate_immutable_release \
    ...
    deploy/finalize-pending-update.sh
validate_immutable_release \
    target "$TRUSTED_TARGET_RELEASE" "$PENDING_TARGET" ''

# The verified recovery release, never the historical target, owns the current
...
release_txn_create_start_authorization \
    "$TRANSACTION_ROOT" "$TRANSACTION_RUNTIME_ROOT" \
    "$RELEASE_TRANSACTION_FD" "$pending_token" update "$pending_snapshot" \
    || die "cannot authorize the exact pending-target controlled starts"
...
release_txn_remove_start_authorization \
    "$TRANSACTION_ROOT" "$TRANSACTION_RUNTIME_ROOT" \
    "$RELEASE_TRANSACTION_FD" "$pending_token" update "$pending_snapshot" \
    || die "cannot remove exact pending-target start authorization"
start_authorization_created=0
