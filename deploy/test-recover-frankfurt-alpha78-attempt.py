...
spec = importlib.util.spec_from_file_location('recovery', REPO / 'deploy/recover-frankfurt-alpha78-attempt.py')
...
class RecoveryTests(unittest.TestCase):
    def setUp(self):
        ...
        self.stack.enter_context(patch.object(recovery, 'INCIDENT_IDENTITIES_REVIEWED', True))
        ...
        root = recovery.RELEASES / ('ae881fc2d55b-' + '0' * 24)
        ...
        self.write(root / 'release.commit', b'ae881fc2d55b7bb6b6bd2674a1cf893d38e71539\n')
    ...
    def test_existing_evidence_refuses_before_marker_removal(self):
        recovery.EVIDENCE.mkdir(mode=0o700)
        self.rejected()

    def test_cross_device_evidence_refuses_before_marker_removal(self):
        original_stat = Path.stat
        def different_device(path, *args, **kwargs):
            value = original_stat(path, *args, **kwargs)
            if path == recovery.STAGE:
                fields = list(value)
                fields[2] += 1
    ...
    def test_unreviewed_draft_never_runs(self):
        with patch.object(recovery, 'INCIDENT_IDENTITIES_REVIEWED', False):
            self.rejected()
    ...
    def test_wrong_incident(self):
        self.write(recovery.TX / 'active', self.marker.replace(b'20260913T221708Z', b'20260913T221709Z'))
