#!/usr/bin/env python3
"""Restore existing Alpha75 services after the exact Alpha78 receipt failure.
Yalnız Alpha78 işlem kaydı hatasının mevcut Alpha75 servislerini yeniden açar.
...
# Enabled only after the owner supplies this incident's coordinator ledger.
# Kullanıcı bu olayın koordinatör kaydını paylaştıktan sonra etkinleştirilir.
INCIDENT_IDENTITIES_REVIEWED = True
...
STAGE = SNAPS / '.release-snapshot.incomplete.1225232.7c4ac30c46ae27c9087fb3612c715284'
CHILD = STAGE / NAME
RELEASES = Path('/var/backups/celikpanel/releases')
EVIDENCE = Path('/var/backups/celikpanel/frankfurt-recovery-20260913T221708Z')
...
COORDINATORS = (b'celikpanel-agent.service\tactive\t1156220\t43004822\n'
                b'celikpanel-panel.service\tactive\t1156319\t43004899\n')
...
def verify_stopped():
    ...
    for row in COORDINATORS.decode('ascii').splitlines():
        _unit, _state, pid, started = row.split('\t')
...
def held_lock(path, uid, gid, mode):
    ...
    # Wait only for the exact transaction lock, while the short automatic
    # recovery attempt exits. No service or update is started while waiting.
    # Kısa otomatik kurtarma denemesi çıkarken yalnız tam işlem kilidini bekle;
    # beklerken servis veya güncelleme başlatılmaz.
    deadline = time.monotonic() + (10 if path == TX / 'transaction.lock' else 0)
    try:
        while True:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise RuntimeError('Another operation still holds the recovery lock; no recovery changes were made')
                time.sleep(0.1)
        verify_lock_identity(path, fd)
        return fd
...
def find_guard():
    ...
    candidates = list(RELEASES.glob('ae881fc2d55b-*'))
    require(len(candidates) == 1, 'Need exactly one retained Alpha78 release')
    root = candidates[0]
    ...
    require((root / 'release.commit').read_bytes() == b'ae881fc2d55b7bb6b6bd2674a1cf893d38e71539\n', 'Wrong retained release commit')
    return root
...
def run(restore=False):
    require(INCIDENT_IDENTITIES_REVIEWED, 'This recovery draft awaits reviewed coordinator identities; no changes were made')
    ...
    try:
        ...
        trusted_chain(EVIDENCE.parent)
        require(not os.path.lexists(EVIDENCE), 'Recovery evidence already exists; inspect it before retrying')
        require(STAGE.stat().st_dev == EVIDENCE.parent.stat().st_dev,
                'Evidence must be on the same filesystem')
