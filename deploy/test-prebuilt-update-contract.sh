...
require_literal 'VALIDATED_RELEASE_TREE=$tree'
...
require_literal 'read_pending_rollback_route'
require_literal 'release_txn_validate_pending_token \'
require_literal '"$TRANSACTION_ROOT" "$pending_token" rollback "$pending_snapshot"'
require_literal 'pending rollback snapshot checksum verification failed'
require_literal 'pending rollback recovery requires exactly one retained immutable transaction release'
require_literal 'validate_release_tree "$transaction_release" 1'
require_literal '"$transaction_tree" == "$snapshot_tree"'
require_literal 'bash "$FINAL_RELEASE/deploy/finalize-pending-rollback.sh"'
require_literal '"--trusted-recovery-release=$FINAL_RELEASE"'
require_literal '"--trusted-transaction-release=$PENDING_ROLLBACK_TRANSACTION_RELEASE"'
require_literal '|| die "verified pending rollback finalization failed"'
require_literal 'pending rollback finalizer is missing from the recovery release'
require_literal 'published pending rollback finalizer is missing or unsafe'
...
require_before 'NEW_RELEASE_COMMIT=$VALIDATED_RELEASE_COMMIT' 'if finalize_pending_rollback_if_present; then'
require_before 'if finalize_pending_rollback_if_present; then' 'bash "$FINAL_RELEASE/update.sh" --normal'
