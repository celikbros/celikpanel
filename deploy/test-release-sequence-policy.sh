...
[[ "$policy_version" == v0.1.0-alpha.58 ]] || die 'tracked version must be v0.1.0-alpha.58'
[[ "$policy_current" == 58 ]] || die 'tracked current sequence must be 58'
[[ "$policy_previous" == 57 ]] || die 'tracked previous sequence must be 57'
[[ "$policy_previous_version" == v0.1.0-alpha.57 ]] || die 'tracked previous version must be v0.1.0-alpha.57'
[[ "$policy_previous_commit" == 4e48d21869cede182f88dab95823488fdb0d2e94 ]] \
  || die 'tracked previous commit must be the immutable Alpha57 release commit'
