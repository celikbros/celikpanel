...
func TestSSLLineageIdentityMigrationBackfillsLegacyCertificateHistory(t *testing.T) {
	database := newDatabaseAtMigrationVersion(t, 19)
	raw := database.GetDB()
	...
	if err := database.RunMigrations(); err != nil {
		t.Fatalf("run v19 to v20 migration: %v", err)
	}

	var version20 int
	if err := raw.QueryRow(`
		SELECT COUNT(*) FROM schema_migrations WHERE version = 20`).Scan(&version20); err != nil {
		t.Fatalf("query migration 20 state: %v", err)
	}
	if version20 != 1 {
		t.Fatalf("migration 20 applied rows = %d, want 1", version20)
	}
...
func assertSSLIdentityColumnsAbsent(t *testing.T, raw *sql.DB) {
	...
	if err != nil {
		t.Fatalf("inspect v19 ssl_certificates schema: %v", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if name == "lineage_name" || name == "acme_provider_id" {
			t.Fatalf("v19 unexpectedly contains %q", name)
		}
	}
