...
test('dashboard firewall enable CTA requests confirmation and persistence stays explicit', () => {
  ...
  assert.match(dashboardSource, /fw\?\.enabled && fw\.persistence_state !== 'ready'/);
  assert.match(dashboardSource, /action: t\('dashboard\.saveFirewall'\)/);
  assert.doesNotMatch(dashboardSource, /<StartGuide/);
  assert.match(dashboardSource, /<ServerSetupDashboardNotice/);
});
