...
export function MariaDBSettings({ configPath }: MariaDBSettingsProps) {
    ...
    if (loading) return <div className="p-8 text-center text-slate-500">Loading MariaDB configuration...</div>;
                ...
                {filteredSections.length === 0 ? (
                    ...
                    filteredSections.map((section) => (
                        <div key={section.title} className="border border-slate-800 rounded-lg overflow-hidden bg-slate-900/20">
                            ...
                            >
                                <span className="font-bold text-sm text-slate-300 uppercase tracking-wide">
                                    [{section.title}]
                                    <span className="text-slate-600 text-xs ml-2">({section.items.length})</span>
                                </span>
                            ...
                            {(expandedSections[section.title] || searchTerm) && (
                                <div className="p-4 border-t border-slate-800 grid grid-cols-1 gap-4">
                                    {section.items.map((item, iIdx) => (
                                        <div key={item.key + iIdx} className={`group flex flex-col sm:flex-row sm:items-start gap-4 p-3 rounded-lg border ${item.enabled ? 'border-slate-800 bg-slate-900/40' : 'border-dashed border-slate-800/50 opacity-60'}`}>
                                            <div className="pt-2">
                                                <input
                                                    type="checkbox"
