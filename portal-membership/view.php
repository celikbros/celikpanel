...
$messages=[
 ...
 'key_not_saved'=>['Anahtar görüntülenemiyor. Lisanslarınızdan yeni anahtar oluşturabilirsiniz; bitiş tarihi değişmez.','The key cannot be displayed. You can create a replacement from your licenses; the expiry date stays the same.'],
...
$titles=['terms'=>t('Kullanım koşulları ve veri açıklaması','Terms and data notice'),'login'=>t('Hesabınıza giriş yapın','Sign in to your account'),'register'=>t('CelikPanel hesabınızı oluşturun','Create your CelikPanel account'),
 ...
 'home'=>t('Lisanslarınız','Your licenses'),'release'=>t('Sunucu / IP değiştirin','Change server / IP'),'reveal'=>t('Lisans anahtarınız','Your license key'),'rotate'=>t('Lisans anahtarını değiştirin','Replace your license key'),'renew'=>t('Lisansınızı uzatın','Renew your license')];
...
<?php if ($issued): ?><section class="member-key" aria-labelledby="new-key"><h2 id="new-key"><?=t('Lisans anahtarınız','Your license key')?></h2><p><?=t('Kurulumdan sonra sunucunuzdaki panele yönetici olarak giriş yapın ve bu anahtarı Lisans ekranına yapıştırın. Anahtarı daha sonra hesabınızdan tekrar görüntüleyebilirsiniz.','After installation, sign in to your server panel as administrator and paste this key on the License screen. You can view the key again from your account.')?></p><code id="license-key"><?=h($issued['key'])?></code><button type="button" data-copy="license-key"><?=t('Anahtarı kopyala','Copy key')?></button><span id="copy-status" role="status"></span></section><?php endif ?>
...
<article class="member-license"><div><h2><?=h($l['hostname']?:t('Aktivasyon bekliyor','Awaiting activation'))?></h2><p><span class="member-state"><?= $expired?t('Süresi doldu','Expired'):($l['server_id']?t('Aktif','Active'):t('Sunucuya bağlanmadı','Not bound to a server')) ?></span> <code>…<?=h($l['key_suffix'])?></code></p><?php if ($l['server_ip']): ?><p><?=t('Eşleşen IP','Bound IP')?>: <code><?=h($l['server_ip'])?></code></p><?php endif ?></div><dl><dt><?=t('Bitiş tarihi (UTC)','Expires (UTC)')?></dt><dd><?=$l['expires']?h(gmdate('Y-m-d H:i',(int)$l['expires'])):t('Tarih belirlenemedi','Date unavailable')?></dd></dl><div class="member-actions"><?php if ($l['key_available']): ?><a href="?action=reveal&amp;id=<?=h($l['id'])?>"><?=t('Anahtarı göster','Show key')?></a><?php else: ?><span><?=t('Bu eski lisansın anahtarı görüntülenemiyor.','This older license key cannot be displayed.')?></span><?php endif ?><?php if ($l['server_id']): ?><a href="?action=release&amp;id=<?=h($l['id'])?>"><?=t('Sunucu / IP değiştir','Change server / IP')?></a><?php endif ?><a href="?action=rotate&amp;id=<?=h($l['id'])?>"><?=$l['key_available']?t('Anahtarı değiştir','Replace key'):t('Yeni anahtar oluştur','Create replacement key')?></a><?php if ($l['expires'] && (int)$l['expires']<=$service->now()+30*86400): ?><a href="?action=renew&amp;id=<?=h($l['id'])?>"><?=t('Ücretsiz yenile','Renew for free')?></a><?php endif ?></div></article><?php endforeach ?></div>
<section class="member-install"><h2><?=t('Sunucunuza kurun','Install on your server')?></h2><p><?=t('Desteklenen temiz bir Linux sunucusunda çalıştırın. Terminalde yönetici hesabınızı oluşturun; kurulum bitince panele giriş yapıp lisans anahtarınızı girin. Yönetici parolası bu websitesiyle paylaşılmaz.','Run on a supported clean Linux server. Create your administrator account in the terminal, then sign in to the panel and enter your license key. The administrator password is not shared with this website.')?></p><pre><code id="install-command">curl -fL --proto '=https' -o celikpanel-install.sh https://celikpanel.net/get.sh &amp;&amp;
...
<?php elseif (in_array($action,['release','renew','reveal','rotate'],true) && $member): ?>
<?php $descriptions=[
 ...
 'release'=>t('Sunucu eşlemesi kaldırılacak. Aynı anahtarı yeni sunucuda kullanın; bitiş tarihi değişmez. Güncel panel sürümlerinde eski sunucunun erişimi en geç 60 saniye içinde kapanır. Eski sürümleri önce güncelleyin.','The server binding will be released. Use the same key on the new server; its expiry date stays the same. On current panel versions, the old server loses access within 60 seconds. Update older versions first.'),
 'rotate'=>t('Mevcut anahtar iptal edilip yenisi oluşturulacak. Sunucu eşlemesi ve bitiş tarihi korunur. Yeni anahtarı panelin Lisans ayarlarına girin. Güncel panel sürümlerinde eski doğrulama en geç 60 saniye içinde geçersiz olur. Eski sürümleri önce güncelleyin.','Your current key will be replaced. The server binding and expiry date stay the same. Enter the new key in the panel’s License settings. On current panel versions, the previous verification becomes invalid within 60 seconds. Update older versions first.'),
