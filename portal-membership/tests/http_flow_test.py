...
check('yalnızca şimdi' in body,'one-time key notice')
_,next_body,_=request()
check(key not in next_body,'key not redisplayed')
def api(action,data):
...
check(code==400 and other['error']=='license_in_use','other server rejected')
