...
$oldKey=$license['key'];$oldExpiry=$s->licenses($member['id'])[0]['expires'];
$license=$s->rotate($member['id'],$license['id'],$password);
check($license['key']!==$oldKey,'explicit replacement creates a new key');
check($s->licenses($member['id'])[0]['expires']===$oldExpiry,'legacy replacement preserves expiry');
check($s->reveal($member['id'],$license['id'],$password)['key']===$license['key'],'replacement is saved automatically');
...
$s->refresh($ipLicense['id'],$other,$reinstalled['activation_token'],'8.8.4.4');
$beforeRotation=$s->activate($ipLicense['key'],$server,'ip.example.com','8.8.4.4');
...
rejects(fn()=>$s->refresh($ipLicense['id'],$server,$beforeRotation['activation_token'],'8.8.4.4'),'invalid_activation');
